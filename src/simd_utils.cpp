...
std::optional<std::size_t> scalar_find_next(std::span<const uint64_t> words, std::size_t start) noexcept
{
...
std::optional<std::size_t> scalar_find_prev(std::span<const uint64_t> words, std::size_t start) noexcept
{
    if (words.empty()) {
        return std::nullopt;
    }
...
#if PARVEB_HAS_X86
PARVEB_TARGET_AVX2 std::optional<std::size_t> avx2_find_next(std::span<const uint64_t> words,
                                                             std::size_t start) noexcept
{
    std::size_t i = start;
    constexpr std::size_t kStride = 4;
    for (; i + kStride <= words.size(); i += kStride) {
        __m256i chunk = _mm256_loadu_si256(reinterpret_cast<const __m256i *>(words.data() + i));
...
PARVEB_TARGET_AVX2 std::optional<std::size_t> avx2_find_prev(std::span<const uint64_t> words,
                                                             std::size_t start) noexcept
{
    if (words.empty()) {
        return std::nullopt;
    ...
    while (true) {
        ...
        __m256i chunk = _mm256_loadu_si256(reinterpret_cast<const __m256i *>(words.data() + base));
        if (!_mm256_testz_si256(chunk, chunk)) {
...
PARVEB_TARGET_SSE2 std::optional<std::size_t> sse2_find_next(std::span<const uint64_t> words,
                                                             std::size_t start) noexcept
{
    std::size_t i = start;
    ...
    for (; i + kStride <= words.size(); i += kStride) {
        __m128i chunk = _mm_loadu_si128(reinterpret_cast<const __m128i *>(words.data() + i));
...
PARVEB_TARGET_SSE2 std::optional<std::size_t> sse2_find_prev(std::span<const uint64_t> words,
                                                             std::size_t start) noexcept
{
    if (words.empty()) {
        return std::nullopt;
    ...
    while (true) {
        ...
        __m128i chunk = _mm_loadu_si128(reinterpret_cast<const __m128i *>(words.data() + base));
...
std::optional<std::size_t> find_next_nonzero(std::span<const uint64_t> words,
...
std::optional<std::size_t> find_prev_nonzero(std::span<const uint64_t> words,
                                             std::size_t start_word) noexcept
{
    if (words.empty()) {
        return std::nullopt;
